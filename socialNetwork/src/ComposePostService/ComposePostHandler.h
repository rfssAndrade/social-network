...
Creator ComposePostHandler::_ComposeCreaterHelper(
    ...
    auto curr_context = context::RuntimeContext::GetCurrent();
    options.parent   = GetSpan(prop->Extract(carriermap, curr_context))->GetContext();
     auto span = trace::Provider::GetTracerProvider()->GetTracer("composetracer")->StartSpan("OT-compose_creator_client",options);
    auto scope = trace::Provider::GetTracerProvider()->GetTracer("composetracer")->WithActiveSpan(span);
    prop->Inject(carriermap, context::RuntimeContext::GetCurrent());
...
TextServiceReturn ComposePostHandler::_ComposeTextHelper(
    ...
    auto curr_context = context::RuntimeContext::GetCurrent();
    options.parent   = GetSpan(prop->Extract(carriermap, curr_context))->GetContext();
     auto span = trace::Provider::GetTracerProvider()->GetTracer("composetracer")->StartSpan("OT-compose_text_client",options);
    auto scope = trace::Provider::GetTracerProvider()->GetTracer("composetracer")->WithActiveSpan(span);
    prop->Inject(carriermap, context::RuntimeContext::GetCurrent());
  ...
  try {
    text_client->ComposeText(_return_text, req_id, text, carriermap.headers_);
  } catch (...) {
